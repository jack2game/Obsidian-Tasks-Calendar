...
```dataviewjs
const fileorfolder = `"${dv.current().file.path}"`;
//const fileorfolder = '"Template/Gantt Chart.md"';
...
const Mermaid = `gantt
        title Project Timeline and Tasks
        dateFormat  YYYY-MM-DD
        axisFormat %b %e
        `;
    //let pages = [dv.current()];
    let pages = dv.pages(fileorfolder);
...
//	}
    ...
    dv.span("[View this chart in a browser](https://mermaid.ink/img/"+btoa(unescape(encodeURIComponent(Mermaid+ganttOutput)))+")")
